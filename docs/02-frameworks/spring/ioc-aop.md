...
- **IOC**：控制反转，把对象创建、依赖管理、生命周期回调交给 Spring 容器；开发者关注业务对象关系。
- **DI**：依赖注入，是 IOC 的主要实现方式，包括构造器注入、Setter 注入、字段注入。
- **BeanDefinition**：Bean 的元信息，包含 class、scope、lazy、dependsOn、init/destroy、属性依赖等。
- **AOP**：面向切面编程，把日志、事务、权限、监控等横切逻辑从业务代码中抽离，通过代理织入。
- **事务**：Spring 声明式事务本质是 AOP + PlatformTransactionManager + 数据库事务。

### 面试官想考什么

1. 能否讲清楚 Bean 从扫描到可用的完整生命周期。
2. 是否知道三级缓存解决的是单例 Setter/字段注入的循环依赖，不是所有循环依赖。
3. 是否理解 AOP 代理边界：自调用、final/private 方法、非 Spring Bean 都可能不生效。
4. 能否把事务传播行为与业务场景结合，而不是机械背枚举。

### 标准回答

Spring IOC 启动时先读取配置或注解扫描，生成 BeanDefinition 注册到 BeanFactory；创建 Bean 时先实例化对象，再进行属性填充、Aware 回调、BeanPostProcessor 前置处理、初始化方法、BeanPostProcessor 后置处理，最后放入单例池。AOP 通常在后置处理阶段为符合切点的 Bean 创建代理对象，调用代理方法时进入拦截器链完成事务、日志等增强。

循环依赖方面，Spring 通过三级缓存提前暴露单例对象引用：一级缓存放完整 Bean，二级缓存放早期对象，三级缓存放 ObjectFactory 用于生成早期代理对象。它只能解决单例、非构造器注入的循环依赖；构造器循环依赖、prototype 循环依赖仍然解决不了。

事务方面，`@Transactional` 通过代理开启、提交、回滚事务。传播行为决定当前方法和已有事务的关系，隔离级别由数据库实现，回滚默认针对 RuntimeException 和 Error。

### 深挖追问

- **为什么构造器循环依赖解决不了？** 构造器实例化前必须先拿到依赖对象，无法提前暴露“半成品”。
- **为什么事务自调用失效？** 同类内部方法调用走 `this`，没有经过 Spring 代理对象。
- **JDK 动态代理和 CGLIB 怎么选？** 有接口默认可用 JDK 动态代理；无接口通常用 CGLIB 生成子类，final 类/方法无法被增强。
- **BeanFactory 和 ApplicationContext 区别？** ApplicationContext 是 BeanFactory 的增强，提供国际化、事件、资源加载、自动注册后置处理器等能力。

### 示例/实战场景

- 订单服务调用库存、积分、消息发送时，事务边界应只覆盖本地数据库操作，外部 RPC 或 MQ 投递建议用本地消息表/Outbox，避免长事务。
- 线上发现 `@Transactional` 不回滚，排查顺序：是否 public 方法、是否被代理调用、异常是否被捕获、异常类型是否匹配 rollbackFor、事务管理器是否正确、数据库引擎是否支持事务。
- 多个实现类注入冲突时，可用 `@Primary`、`@Qualifier` 或按名称注入，但更推荐从业务建模上避免模糊依赖。

### 易错点/总结

- IOC 的关键不是“少写 new”，而是统一管理对象生命周期和依赖关系。
- AOP 不是编译期修改源码，Spring 常见实现是运行期代理。
- 三级缓存不是为了解决所有循环依赖，而是为了解决“代理对象提前暴露”的一致性问题。
- 事务不是 Spring 独立实现的，最终依赖数据库连接和数据库事务能力。
