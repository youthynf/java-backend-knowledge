...
调节缓冲区大小：发送端缓冲区时自行调节的，通过tcp_wmem参数指定初始值、动态范围最小值和动态范围最大值；接收端缓冲区则可以根据系统空闲内存的大小来调节接收窗口，通过tcp_rmem参数指定初始值、动态范围最小值和动态范围最大值。不同的是，发送缓冲区的自动调节功能是自动开启的，而服务端的缓冲区则需���配置tcp_moderate_rcvbuf=1来开启自动调节功能。
