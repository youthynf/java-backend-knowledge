...
从面试角度看，**XSS跨站脚本攻击是什么？** 可以放在“网络安全”这一类知识中理解。复习时不要只背定义，要能说清：它解决什么问题、依赖哪些前提、正常流程是什么、异常情况下系统会怎样退化或恢复。
...
> 先建立威胁模型：攻击者控制什么输入、越过什么信任边界、造成什么影响。 追问会考漏洞成因、攻击链路、检测方式、防御手段和防御局限。 对于“XSS跨站脚本攻击是什么？”，回答时建议先给一句话定义，再按“工作流程/关键机制 → 典型场景 → 风险与优化”展开，最后补充一两个线上实践点。
