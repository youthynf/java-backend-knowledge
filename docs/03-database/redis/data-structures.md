...
这篇属于 **Redis 数据结构** 的综合复习文，核心要抓住：redisObject、SDS、dict、quicklist/listpack、intset、skiplist 与命令复杂度。面试中要从“定义”进一步讲到“为什么这样设计、如何落地、线上怎么排查”。

### 面试官想考什么

面试官通常想确认：是否能从类型语义讲到底层编码、复杂度、适用场景和 big key 风险；同时会关注你是否能结合数据量、并发量、业务一致性和线上稳定性做取舍。

### 标准回答

Redis 数据类型的选择要围绕访问模式：整体读写用 String，字段级读写用 Hash，去重用 Set，排序排行榜用 ZSet，消息流用 Stream。底层编码会随元素数量和大小变化，影响内存与性能。
...
例如排行榜用 ZSET 的 `ZADD/ZREVRANGE`，共同关注用 SET 的 `SINTER`，签到用 Bitmap；上线前要估算 key 数、value 大小、TTL 和最坏命令复杂度。
