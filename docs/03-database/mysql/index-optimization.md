...
这篇属于 **MySQL 索引与查询优化** 的综合复习文，核心要抓住：索引结构、联合索引、执行计划、慢 SQL、分页与统计优化。面试中要从“定义”进一步讲到“为什么这样设计、如何落地、线上怎么排查”。

### 面试官想考什么

面试官通常想确认：索引设计是否服务查询模式、是否能通过 EXPLAIN 验证、是否知道写入代价和失效场景；同时会关注你是否能结合数据量、并发量、业务一致性和线上稳定性做取舍。

### 标准回答

索引优化不是简单加索引，而是让查询尽量少扫描、少回表、少排序、少锁等待。先定位慢 SQL，再结合 where/order by/group by/返回列和数据分布设计联合或覆盖索引，最后压测验证收益。

回答时建议按以下结构组织：

1. 先说明核心机制和适用场景；
2. 再解释底层结构、执行成本或时间复杂度；
3. 最后补充生产环境的风险控制和排查方法。

### 深挖追问

- 数据量扩大 10 倍后，瓶颈会出现在 CPU、内存、磁盘、网络还是锁等待？
- 这个方案对版本、配置、索引/编码、客户端使用方式有什么依赖？
- 如果线上变慢或抖动，你会先看哪些日志、指标和命令输出？

### 示例 / 实战场景

例如列表查询 `where tenant_id=? and status=? order by created_at desc limit 20`，通常优先考虑 `(tenant_id,status,created_at)`，再根据返回列判断是否做覆盖索引。

```sql
EXPLAIN SELECT * FROM your_table WHERE tenant_id=? AND status=? ORDER BY created_at DESC LIMIT 20;
```

### 易错点

- 把“能用”当成“适合用”，没有说明边界和代价。
- 只背结论，不会用执行计划、慢日志、监控指标或 Redis 诊断命令验证。
- 忽略大 key、热 key、深分页、索引维护成本、版本差异等生产问题。

### 一句话总结

综合题的关键是把知识点串成方法论：**先判断访问模式，再选择结构/索引，最后通过可观测手段验证并持续治理**。
