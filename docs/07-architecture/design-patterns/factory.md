...
从面试角度看，**工厂模式** 可以放在“设计模式”这一类知识中理解。复习时不要只背定义，要能说清：它解决什么问题、依赖哪些前提、正常流程是什么、异常情况下系统会怎样退化或恢复。
...
> 先说明变化点与稳定点，再讲类之间职责边界以及调用方如何依赖抽象。 追问常考适用场景、优缺点、与相似模式差异，以及在 Spring/JDK 中的真实应用。 对于“工厂模式”，回答时建议先给一句话定义，再按“工作流程/关键机制 → 典型场景 → 风险与优化”展开，最后补充一两个线上实践点。
