...
### 核心概念
- ThreadPoolExecutor线程池任务执行流程是怎么样的？ 属于 Java 并发基础，核心是线程安全、可见性、原子性、有序性以及 JUC 对这些问题的工程化封装。
...
### 标准回答
回答 ThreadPoolExecutor线程池任务执行流程是怎么样的？ 时，应先界定概念，再说明状态流转或实现机制，并结合锁、volatile、CAS、阻塞队列/线程池等组件解释实际行为。
