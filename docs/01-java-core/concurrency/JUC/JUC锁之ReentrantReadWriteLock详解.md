...
类的内部类
ReentrantReadWriteLock有五个内部类，分别是Sync，NonfairSync，FairSync、ReadLock，WriteLock，这五个内部类之间也是相互关联的，Sync继承自AQS、NonfairSync和FairSync均继承自Sync类；ReadLock实现了Lock接口、WriteLock也实现了Lock接口。
•  Sync类
Sync抽象类继承自AQS抽象类，Sync类提供了对ReentrantReadWriteLock的支持。

abstract static class Sync extends AbstractQueuedSynchronizer {}
Sync定义了两个内部类，分别是HoldCounter和ThreadLocalHoldCounter。前者的count属性记录了重入的次数和tid唯一标识线程ID；而后者是一个本地线程计数器，它重写了ThreadLocal的initialValue方法保证HoldCounter对象与线程绑定。
•  Sync类的核心函数
shareCount()：获取读锁的数量。state的前16位表示读锁数量，通过将state右移16位获得。

static int sharedCount(int c)    { return c >>> SHARED_SHIFT; }

exclusiveCount()：获取写锁数量。state的后16位表示写锁数量。通过将state和（2^16-1）相与获得。

static int exclusiveCount(int c) { return c & EXCLUSIVE_MASK; }

tryRelease()：此方法用于释放写锁，判断当前锁若不是独占锁，则抛异常；如果是写锁，则计算写锁数量，如果为0表示释放锁成功，否则表示资源还被占有。
...
fullTryAcquireShared()：在tryAcquireShared函数中，如果下列三个条件不满足(读线程是否应该被阻塞、小于最大值、比较设置成功)则会进行fullTryAcquireShared函数中，它用来保证相关操作可以成功。
...
这篇文章的主题是 **JUC锁之ReentrantReadWriteLock详解**。复习时应先给出定义，再说明它在 Java 并发或 JVM 体系中的位置，最后结合使用场景、限制条件和常见误区展开。
