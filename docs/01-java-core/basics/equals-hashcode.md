...
### 核心概念
- equals 定义逻辑相等，hashCode 用于哈希容器定位。
- equals 为 true 则 hashCode 必须相同；反之不要求。

### 面试官想考什么
- HashMap/HashSet 如何使用二者。
- 为什么重写 equals 必须重写 hashCode。

### 标准回答
哈希容器先用 hashCode 定位桶，再用 equals 精确比较。只重写 equals 会让逻辑相等对象落到不同桶，导致 get/contains/remove 失败。
